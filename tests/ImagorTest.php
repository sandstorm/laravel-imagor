...
it('can generate signed url with basic resize', function () {
    $builder = new Imagor($this->base_url, $this->base_url, $this->signer_type, $this->secret, $this->signer_truncate);
    $url = $builder
...
it('can generate unsafe url when no secret provided', function () {
    $builder = new Imagor($this->base_url, $this->base_url,null, null, null);
    $url = $builder
        ->resize(width: 300, height: 200)
        ->uriFor($this->sample_image_url);
...
it('can generate url with truncated signature', function () {
    $builder = new Imagor($this->base_url, $this->base_url, $this->signer_type, $this->secret, 20);
    $url = $builder
        ->resize(width: 300, height: 200)
        ->uriFor($this->sample_image_url);
...
it('maintains independent state between instances', function () {
    $builder1 = new Imagor($this->base_url, $this->base_url, $this->signer_type, $this->secret, $this->signer_truncate);
    $builder2 = new Imagor($this->base_url, $this->base_url, $this->signer_type, $this->secret, $this->signer_truncate);
